...
const produtos = [
    ...
    {
    id: 2,
    description: 'Notebook Samsung Book E30 Intel Core i3 4GB 1TB - 156” Full HD Windows 10',
...
router.get('/:document', (req: Request, res: Response) =>{
    const produto = produtos.find((prod) => prod.id.toString() === req.params.document);
...
router.delete('/remove/:document', (req: Request, res: Response) => {
    const produtosIndex = produtos.findIndex((produto) => produto.id.toString() === req.params.document);
    if (produtosIndex === -1) {
...
router.put('/:document', (req: Request, res: Response) => {
    ...
    produtos[produtosIndex] = req.body;
